...
export const getWixContent = async () => {
  try {
    ...
    // Extract valid video IDs, filtering out undefined or null values
    const videoIds = videos.items
      .map((v) => v.data?.title)
      .filter((id): id is string => !!id); // Ensures only non-empty strings are kept

    if (videoIds.length === 0) {
      console.log("No valid video IDs found from Wix");
      return { status: 404, message: "No valid video IDs found." };
    }

    // Fetch matching videos from your Prisma database
    ...
    return { status: 404, message: "No matching videos found in the database." };
  } catch (error) {
    console.error("Error in getWixContent:", error);
    return { status: 400, message: "An error occurred while fetching content." };
  }
};
